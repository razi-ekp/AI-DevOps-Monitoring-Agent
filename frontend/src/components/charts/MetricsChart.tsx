...
import { Card } from '../ui';
import { Metric, MetricsChartProps, MetricKey } from '../../types';
...
const TABS: Array<{ key: MetricKey; label: string; color: string; unit: string }> = [
  ...
  return (
    <Card title="Metrics — Real-time" accent="cyan" className="span-2">
      <div className="metric-tabs">
        {TABS.map((t) => (
          <button key={t.key} className={`metric-tab ${active === t.key ? 'active' : ''}`} onClick={() => setActive(t.key)}>
            {t.label}
          </button>
        ))}
      </div>
      <ResponsiveContainer width="100%" height={180}>
        <AreaChart data={data} margin={{ top: 4, right: 4, bottom: 0, left: -20 }}>
          ...
          <Area type="monotone" dataKey={active} stroke={tab.color} strokeWidth={2} fill={`url(#grad-${active})`} dot={false} isAnimationActive={false} />
        </AreaChart>
      </ResponsiveContainer>
    </Card>
  );
